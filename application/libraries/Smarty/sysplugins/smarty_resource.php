...
abstract class Smarty_Resource {
	 ...
	 *
	 * {@internal The loaded source is assigned to $_template->source->content
	 * directly.}}
	 *
	 ...
	 * @throws SmartyException if default template handler is registered but not
	 *         callable
	 */
	...
	public static function load(Smarty $smarty, $type) {
		...
		if ($smarty->loadPlugin ( $_resource_class )) {
			...
			if (class_exists ( $_resource_class, false )) {
				...
				// give it another try, now that the resource is registered
				// properly
	...
	/**
	 * extract resource_type and resource_name from template_resource and
	 * config_resource
	 *
	 * @note "C:/foo.tpl" was forced to file resource up till Smarty 3.1.3
	 * (including).
	 * 
	 * @param string $resource_name
	...
	protected static function parseResourceName($resource_name, $default_resource, &$name, &$type) {
		...
		if (! isset ( $parts [1] ) || ! isset ( $parts [0] [1] )) {
			...
			// or single character before the colon is not a resource type, but
	...
	public static function source(Smarty_Internal_Template $_template = null, Smarty $smarty = null, $template_resource = null) {
		...
		// parse resource_name, load resource handler, identify unique resource
		// name
