...
abstract class Smarty_Internal_TemplateBase extends Smarty_Internal_Data {
	...
	public function fetch($template = null, $cache_id = null, $compile_id = null, $parent = null, $display = false, $merge_tpl_vars = true, $no_output_filter = false) {
		...
		if (! ($_template->caching == Smarty::CACHING_LIFETIME_CURRENT || $_template->caching == Smarty::CACHING_LIFETIME_SAVED) || ! $_template->cached->valid) {
			...
			if (! $_template->source->recompiled && ($_template->caching == Smarty::CACHING_LIFETIME_SAVED || $_template->caching == Smarty::CACHING_LIFETIME_CURRENT)) {
				...
				// rendering (must be done before writing cache file because of
				// {function} nocache handling)
				$_smarty_tpl = $_template;
				try {
					ob_start ();
					eval ( "?>" . $output );
				...
				// var_dump('renderTemplate', $_template->has_nocache_code,
	 ...
	 * @return Smarty_Internal_Templatebase current Smarty_Internal_Templatebase
	 *         (or Smarty or Smarty_Internal_Template) instance for chaining
	 ...
	 *        	or instance of Smarty_Resource, or array of callbacks to
	 *        	handle resource (deprecated)
	 * @return Smarty_Internal_Templatebase current Smarty_Internal_Templatebase
	 *         (or Smarty or Smarty_Internal_Template) instance for chaining
	 */
	 ...
	 * @return Smarty_Internal_Templatebase current Smarty_Internal_Templatebase
	 *         (or Smarty or Smarty_Internal_Template) instance for chaining
	 * @throws SmartyException if any of the methods in $allowed or
	 *         $block_methods are invalid
	 */
	 ...
	 * @return Smarty_Internal_Templatebase current Smarty_Internal_Templatebase
	 *         (or Smarty or Smarty_Internal_Template) instance for chaining
	 * @throws SmartyException if $class_impl does not refer to an existing
	 *         class
	 */
	...
	/**
	 * preg_replace callback to convert camelcase getter/setter to underscore
	 * property names
	 *
	 * @param string $match
	 *        	match string
	...
	public function __call($name, $args) {
		...
		if (isset ( $_prefixes [$first3] ) && isset ( $name [3] ) && $name [3] !== '_') {
			if (isset ( $_resolved_property_name [$name] )) {
				...
				// try to keep case correct for future PHP 6.0 case-sensitive
				// class methods
