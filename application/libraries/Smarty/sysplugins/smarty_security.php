...
/*
 * FIXME: Smarty_Security API - getter and setter instead of public properties
 * would allow cultivating an internal cache properly - current implementation
 * of isTrustedResourceDir() assumes that Smarty::$template_dir and
 * Smarty::$config_dir are immutable the cache is killed every time either of
 * the variables change. That means that two distinct Smarty objects with
 * differing $template_dir or $config_dir should NOT share the same
 * Smarty_Security instance, as this would lead to (severe) performance penalty!
...
class Smarty_Security {
	...
	public function isTrustedPhpFunction($function_name, $compiler) {
		...
		return false; // should not, but who knows what happens to the compiler in
		              // the future?
	}
	...
	public function isTrustedResourceDir($filepath) {
		...
		$_directory = array ();
		while ( true ) {
			// remember the directory to add it to _resource_dir in case we're
			// successful
			$_directory [$directory] = true;
			// test if the directory is trusted
			if (isset ( $this->_resource_dir [$directory] )) {
				// merge sub directories of current $directory into
				// _resource_dir to speed up subsequent lookups
	 ...
	 * "{$PROTOCOL}://{$HOSTNAME}".
	 * So
	 * "http://username:password@hello.world.example.org:8080/some-path?some=query-string"
	 * is reduced to "http://hello.world.example.org" prior to applying the
	 * patters from {@link $trusted_uri}.
	 * 
	 * @param string $uri        	
