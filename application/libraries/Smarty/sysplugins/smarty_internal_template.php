...
class Smarty_Internal_Template extends Smarty_Internal_TemplateBase {
	...
	public function compileTemplateSource() {
		if (! $this->source->recompiled) {
			...
			if ($this->source->components) {
				...
				// $this->properties['file_dependency'][$this->source->uid] =
			// array($this->source->filepath, $this->source->timestamp,
			// $source->type);
			} else {
	...
	/**
	 * This function is executed automatically when a compiled or cached
	...
	public function decodeProperties($properties, $cache = false) {
			...
			foreach ( $this->properties ['file_dependency'] as $_file_to_check ) {
				if ($_file_to_check [2] == 'file' || $_file_to_check [2] == 'php') {
					if ($this->source->filepath == $_file_to_check [0] && isset ( $this->source->timestamp )) {
						...
						// file and php types can be checked without loading the
						// respective resource handlers
		...
		if ($cache) {
			// CACHING_LIFETIME_SAVED cache expiry has to be validated here
			// since otherwise we'd define the unifunc
	...
	/**
	 * Template code runtime function to create a local Smarty variable for
	 * array assignments
	 *
	...
	/**
	 * Template code runtime function to get pointer to template variable array
	 * of requested scope
	 *
	 * @param int $scope
	...
	/**
	 * [util function] counts an array, arrayaccess/traversable or PDOStatement
	 * object
	 *
	 * @param mixed $value        	
	 * @return int the count for arrays and objects that implement countable, 1
	 *         for other objects that don't, and 0 for empty elements
	 */
